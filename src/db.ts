/**
 * SQLite database access layer for the Spanish Financial Regulation MCP server.
 *
 * Schema covers CNMV and Banco de Espana regulatory structure:
 *   - sourcebooks        -- top-level regulatory source directories (CNMV_Circulares, BDE_Circulares, etc.)
 *   - provisions         -- individual circulares, articulos, disposiciones, and guias
 *   - enforcement_actions -- CNMV and BdE sanction decisions and enforcement actions
 *
...
export function checkProvisionCurrency(reference: string): {
  ...
  if (!row) {
    return { reference, status: "desconocido", effective_date: null, found: false };
  }
